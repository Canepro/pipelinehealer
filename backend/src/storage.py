...
class ActivityStorage:
    ...
    async def create_activity(self, activity: ActivityRecord) -> str:
        ...
        await self._activities_container.create_item(body=item)
    ...
    async def update_activity(self, activity: ActivityRecord) -> None:
        ...
        if activity.status in (RemediationStatus.COMPLETED, RemediationStatus.FAILED):
        ...
        await self._activities_container.upsert_item(body=item)
    ...
    async def get_activity(self, activity_id: str) -> ActivityRecord | None:
        ...
        items = [
            item async for item in self._activities_container.query_items(
                query=query,
    ...
    async def get_activities(
        ...
        items = [
            ActivityRecord(**item)
            async for item in self._activities_container.query_items(
                query=query,
                parameters=parameters,
            )
        ]
    ...
    async def get_stats(self) -> DashboardStats:
        ...
        async for item in self._activities_container.query_items(query=status_query):
        ...
        async for item in self._activities_container.query_items(query=failure_query):
        ...
        async for item in self._activities_container.query_items(query=repo_query):
        ...
        avg_duration = 0.0
        async for item in self._activities_container.query_items(query=avg_time_query):
    ...
    async def get_repositories(self) -> list[dict[str, Any]]:
        ...
        repos = [
            item async for item in self._activities_container.query_items(query=query)
        ]
