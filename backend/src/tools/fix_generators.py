...
from ..models import Diagnosis, FailureType, RemediationPlan, RemediationAction
...
class FixGenerators:
    ...
    def _generate_issue_only(
        ...
        repo_name = repository_info.get("full_name", "unknown")
    ...
    async def _generate_dependency_fix(
        ...
        if package_manager == "npm":
            ...
            file_changes.append({
                ...
                "replacement": f"{package_name}=={required_version}" if required_version else package_name,
            })
...
""",
    ...
    async def _generate_test_fix(
        ...
        if is_flaky:
            # For flaky tests, suggest retry or quarantine
            return RemediationPlan(
                action=RemediationAction.CREATE_ISSUE,
                description="Create issue for flaky test investigation",
                issue_title=f"[PipelineHealer] Flaky test detected",
    ...
    async def _generate_build_config_fix(
        ...
        if missing_vars:
            ...
            return RemediationPlan(
                ...
                issue_title=f"[PipelineHealer] Missing environment variables in CI",
...
""",
        ...
        return RemediationPlan(
            ...
            issue_title=f"[PipelineHealer] Build configuration error",
