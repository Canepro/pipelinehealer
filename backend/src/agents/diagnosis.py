    ...
    def _pattern_based_diagnosis(
        ...
        for pattern, description in dep_patterns:
            if re.search(pattern, error_text, re.IGNORECASE):
                ...
                package_match = re.search(r"(?:module|package)[:\s]+['\"]?([a-zA-Z0-9_-]+)", error_text)
                ...
                return Diagnosis(
                    ...
                    suggested_fix=f"Update or install the missing dependency",
                    error_details={
                        "package_name": package_name,
                        "package_manager": "npm" if "npm" in error_text.lower() else "pip",
                    },
                )
        ...
        for pattern, linter, description in lint_patterns:
            if re.search(pattern, error_text, re.IGNORECASE):
                return Diagnosis(
                    ...
                    root_cause=description,
                    is_auto_fixable=True,
                    suggested_fix=f"Run {linter} with --fix flag",
                    error_details={
                        "linter": linter,
                        "violations": [],
                    },
        ...
        for pattern, description in config_patterns:
            if re.search(pattern, error_text, re.IGNORECASE):
                ...
                env_var_match = re.search(r"(?:env|variable|secret)[:\s]+['\"]?([A-Z_]+)", error_text)
